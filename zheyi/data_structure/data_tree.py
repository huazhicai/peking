...
class DataTree(object):
    ...
    def push_source(self, source, value, group_path, priority=0):
        nodes = self.meta.get_node_by_source(source)
        if not nodes:
            print('not nodes', nodes)
            return 
        ...
        for node in nodes:
            ...
            if special_rule:
                getattr(special_rules, special_rule)(self, key_path, DataValue(value, priority), group_path)
            else:
                self.set_value(key_path, DataValue(value, priority), group_path)

    def push_group(self, data, priority=0):
        def recursion_push(_data, group_path, new_group=False):
            if isinstance(_data, dict):
                if new_group:
                    group_path = group_path + [self.new_group()]
                for key, value in _data.items():
                    if isinstance(key, int):
                        self.push_source(key, value, group_path, priority)
                    else:
                        recursion_push(value, group_path)
            elif isinstance(_data, list):
                for item in _data:
                    recursion_push(item, group_path, True)
            else:
                print('push_group', _data)
